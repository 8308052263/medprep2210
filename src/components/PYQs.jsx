...
const PYQs = () => {
  ...
  const filteredData =
    selectedTopic === "All"
      ? PYQData
      : PYQData.filter((q) => q.topic === selectedTopic);
  const current = filteredData[index];

  const handleAnswer = (option) => {
    setSelected(option);
    const isCorrect = option === current.answer;
    if (isCorrect) confetti(); // 🎉 Show firecracker confetti
    setScore((prev) => ({
  ...
  if (!current)
    return <div className="p-6 md:ml-20 text-center">No PYQs available.</div>;
